document.addEventListener('DOMContentLoaded', () => {
    ...
    // Localize esta função no seu script.js e substitua-a por esta versão
function runFinancialPlan() {
    ...
    try {
        ...
        // Após gerar o plano com sucesso, marcamos no localStorage que já foi usado.
        localStorage.setItem('planningGenerated', 'true');
        // <<< NOSSO DETETIVE ESTÁ AQUI >>>
        console.log('SUCESSO: O plano foi gerado. Tentando salvar no localStorage...');
        localStorage.setItem('planningGenerated', 'true');
        console.log('SALVO: O valor "true" foi salvo na chave "planningGenerated".');
...
// Localize e REMOVA a linha abaixo que existe no seu código:
// document.getElementById('generate-dashboard-btn').addEventListener('click', () => { runFinancialPlan(); });

// ===================================================================
// INÍCIO DO NOVO CÓDIGO - Adicione isso no final do seu script.js
// ===================================================================

const generateButton = document.getElementById('generate-dashboard-btn');
const blockerPopup = document.getElementById('blocker-popup');
const closePopupButton = document.querySelector('.popup-close');

// Função para mostrar o pop-up
function showBlockerPopup() {
    blockerPopup.classList.remove('hidden');
}

// Função para esconder o pop-up
function hideBlockerPopup() {
    blockerPopup.classList.add('hidden');
}

// Função que decide o que fazer quando o botão principal é clicado
// ===================================================================
//  SUBSTITUA A VERSÃO ANTIGA DESTA FUNÇÃO PELA NOVA VERSÃO ABAIXO
// ===================================================================

// Função que decide o que fazer quando o botão principal é clicado
function handleGenerateClick() {
    console.log('CLIQUE: O botão "Gerar Planejamento" foi clicado.');
    // 1. Verifica no localStorage se a chave 'planningGenerated' existe e é 'true'
    const hasGenerated = localStorage.getItem('planningGenerated');
 console.log('VERIFICAÇÃO: Valor lido do localStorage para "planningGenerated" foi:', hasGenerated);
    if (hasGenerated === 'true') {
        // 2. Se já usou, bloqueia o acesso e mostra o pop-up
        console.log("Usuário já gerou um planejamento. Bloqueando acesso e mostrando pop-up.");
        console.log('RESULTADO: BLOQUEADO. O usuário já usou a ferramenta.');
        // --- INÍCIO DA ALTERAÇÃO ---
        // Esconde todas as seções e volta para a tela inicial
        document.querySelectorAll('section.card').forEach(s => s.classList.add('hidden'));
        document.getElementById('profile').classList.remove('hidden');
        window.scrollTo(0, 0); // Rola a página para o topo
        // --- FIM DA ALTERAÇÃO ---

        showBlockerPopup(); // Mostra o pop-up de aviso
    } else {
        // 3. Se for a primeira vez, roda a função normal para gerar o plano
        console.log("Primeira utilização. Gerando planejamento.");
        runFinancialPlan();
    }
}

// Adiciona o gatilho principal ao botão de gerar planejamento
generateButton.addEventListener('click', handleGenerateClick);

// Adiciona os gatilhos para fechar o pop-up
closePopupButton.addEventListener('click', hideBlockerPopup);
blockerPopup.addEventListener('click', (event) => {
    // Fecha o pop-up se o clique for no fundo escuro (overlay) e não na caixa de conteúdo
    if (event.target === blockerPopup) {
        hideBlockerPopup();
    }
});